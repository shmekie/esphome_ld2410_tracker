...
bool LD2410Tracker::parse_frame(std::vector<uint8_t> &stream,
  ...
  // Ensure header at start; if not, resync.
  ...
  size_t full_len = 6 + len + 4; // header + len + data + tail

  if (stream.size() < full_len) return false;
  ...
  if (!(stream[full_len-4] == TAIL[0] &&
    ...
    // corrupt frame, drop first byte and retry later
  ...
  if (type != 0x02) {
    // Not normal target data; drop frame
  ...
  // Heuristic angle: map energy difference to 0..90°
...
void LD2410Tracker::loop() {
  ...
  while (true) {
    ...
    if (!parse_frame(stream, detected, angle_deg, dist_cm))
      break;

    // Update ESPHome globals (user must define these in YAML)
    // int: object_present
    // float: object_angle_deg, object_distance_cm
    // int: tracking_enabled
    // float: field_start_deg, field_end_deg, steps_per_degree, current_angle_deg

    id(object_present) = detected ? 1 : 0;
    id(object_angle_deg) = angle_deg;
    id(object_distance_cm) = dist_cm;

    if (detected && id(tracking_enabled)) {
      float target = angle_deg;
      if (target < id(field_start_deg)) target = id(field_start_deg);
      if (target > id(field_end_deg)) target = id(field_end_deg);

      int steps = (int) roundf(target * id(steps_per_degree));
      id(tracker_stepper).set_target(steps);
      id(current_angle_deg) = target;
    }
  }
}
